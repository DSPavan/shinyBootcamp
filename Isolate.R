...
server <- function(input,output) { 
  ...
  output$summary <- renderText({
    ...
    paste0('ISOLATED (Slider) Value, Change only after Submit button:  ', isolate(input$n))
  })
  ...
  output$summary2 <- renderText({
    ...
    str <- paste0('NO ISOLATION - Value changes Automatically "', input$text, '"')
    
    # Any sort of expression can go in isolate()
    isolate({
      #str <- paste0(str, ', and AUTO CHANGE is ')
      paste0(str, isolate(input$n))
    })
  })
  
  ## ObserveEvent and isolate , eventreactive difference ?
  }
